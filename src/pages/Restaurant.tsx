...
const MEDVEDJA_VECERJA_SLIDES = [
  { src: '/images/client/restaurant/restaurant-interior-01-1600w.webp', alt: 'Kamnita klet, kjer postrežemo medvedjo večerjo' },
  { src: '/images/client/restaurant/restaurant-food-03-1600w.webp', alt: 'Jed iz divjačine z noto gozda' },
];
...
export default function Restaurant() {
  ...
  return (
    ...
    >
      ...
      {/* Grid: Meso iz slovenskih gozdov */}
      <section className="py-24 bg-brand-cream-soft border-t border-brand-gold/10 relative z-10">
        <motion.div
        ...
        >
          ...
          <motion.div variants={cardVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <ImageSlider
              images={RESTAVRACIJA_SLIDES}
              aspectClassName="aspect-[4/3] md:aspect-[16/11]"
              roundedClassName="rounded-[60px]"
              className="border border-brand-gold/10"
              label="Slike restavracije Pod Slavnikom"
              priority
            />
          </motion.div>
        </motion.div>
      ...
      {/* Signature Event: Medvedja večerja — featured right after the game-meat
          story, since it is the clearest expression of it, not buried in the
          general menu further down the page. */}
      <section id="medvedja-vecerja" className="relative py-24 md:py-28 px-6 bg-brand-bark-deep text-brand-cream overflow-hidden scroll-mt-24">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center space-y-10">
          ...
          <ImageSlider
            images={MEDVEDJA_VECERJA_SLIDES}
            aspectClassName="aspect-[16/10] sm:aspect-[21/9]"
            roundedClassName="rounded-[48px]"
            className="max-w-2xl mx-auto border border-brand-gold/20"
            label="Slike medvedje večerje Pod Slavnikom"
          />
        </div>
      </section>

      {/* Grid: Domača proizvodnja */}
      <section className="py-24 bg-brand-cream-soft border-b border-brand-gold/10 relative z-10">
        <motion.div
          variants={containerVariants}
          initial="hidden"
          whileInView="show"
        ...
        >
          {/* Section 2: Ravioli - Domači izdelki
              No genuine Pod Slavnikom photo of njoki/fuži/ravioli exists yet
              (checked the repository and the full public Instagram grid) — the
              client already rejected one mismatched stock photo here, so this
              stays text-led rather than pairing the copy with another image
              that doesn't actually show the product. */}
          <motion.div variants={cardVariants} className="max-w-3xl mx-auto text-center space-y-8">
            <span className="font-serif text-brand-gold text-sm italic tracking-widest">— 02</span>
            <h2 className="text-3xl md:text-4xl font-serif text-brand-wood leading-tight">
              Domači njoki, fuži <br />
              <span className="italic text-brand-gold font-serif">in ravioli.</span>
            </h2>
            <div className="w-12 h-[1px] bg-brand-gold mx-auto"></div>
            <p className="text-brand-stone leading-relaxed text-base md:text-lg max-w-2xl mx-auto">
              Njoki, fuži in ravioli Pod Slavnikom nastajajo ročno, v naši kuhinji. Nadevi se spreminjajo s sezono — spomladi jih napolnimo z divjimi beluši, jeseni s tartufi. Recepture smo z leti izpopolnjevali in jih danes pripravljamo z enakim spoštovanjem do kakovosti, kot ga namenjamo jedem v naši gostilni.
            </p>
